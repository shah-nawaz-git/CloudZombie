...
estimated_monthly_cost (nullable), cost_confidence_at_observation (added by migration 0002), evidence JSON
```
...
```
run_scan(requested_regions, *, scan_id=None):
  provider = ScanScopedProvider(provider)                 # per-scan thread-safe memoization, including ProviderError instances
  scan = load caller-created running scan by scan_id, or create a new running scan
  ...
  for region in regions (ThreadPoolExecutor, max_workers = settings.max_region_concurrency):
      ...
      for detector in DETECTORS:
          ...
          except Exception as e:                cell = failed/internal_error; log the traceback; continue other detectors
...
`app/remediation/eligibility.py` selects `guarded_remediation` only for open, LOW-risk EBS/EIP findings and returns a typed `ScriptUnavailableReason` for every investigation case. `ScriptService` validates stored identifiers, selects eligibility, and handles single and same-account bulk generation.

Templates are static Bash text constants. The generator returns `GeneratedScript(kind, filename, content, checks, warnings)`, emits a header and `readonly NAME="<validated value>"` assignments, and appends a detector-specific static body. **The generator API accepts typed, validated identifiers only** — it never receives the finding's evidence, tags, title, description or any free text. A final assertion re-validates every interpolated value, audits readonly assignments and printable ASCII, and fails closed.
...
GET    /api/findings/{id}             finding + observations[] + related_findings[]; FindingOut adds resource_age_days,
                                      first_observed_days_ago, last_observed_days_ago, observation_age_days,
                                      days_until_persistent, threshold_days and script eligibility fields
PATCH  /api/findings/{id}             {status: "dismissed"|"open", reason?}
GET    /api/findings/{id}/plan        CleanupPlan
GET    /api/findings/{id}/script      {kind, filename, content, checks[], warnings[], unavailable_reason, unavailable_text}
                                      (?format=raw → text/x-shellscript with attachment filename)
POST   /api/scripts/bulk              {finding_ids: [uuid]} → {content, included[], skipped[{finding_id, reason}]}
GET    /api/history                   per-scan observations/exposure, including resource-type count and exposure breakdowns
GET    /api/coverage                  latest non-running scan coverage matrix, region lists and errors
GET    /api/overview                  mode/identity, latest scan/running state, exposure, finding/risk/persistence totals,
                                      coverage summary and resource-type count/exposure breakdowns
...
| `snap-0a1b2c3d4e5f60001` 200 GiB standard, source `vol-0deadbeef00000001` deleted | 1–5 | Snapshot review, REVIEW |
| `snap-0a1b2c3d4e5f60002` source `vol-0deadbeef00000002` deleted, referenced by `ami-0a1b2c3d4e5f60001` | 1–5 | Known dependency blocks → HIGH |
| `snap-0a1b2c3d4e5f60003` source `vol-0deadbeef00000003` deleted, archive tier, shared with `210987654321` | 1–5 | Shared → HIGH |
...
* Frontend: Vitest + Testing Library + MSW using captured seeded-API fixtures; Playwright Chromium for the end-to-end demo flow against Docker Compose (CI) or local dev servers.
* Remediation security: golden generated scripts, shellcheck for golden and fresh output, hostile-metadata injection tests, behavioural tests under Bash with a fake `aws`, and bulk skip/confirmation cases.
* Static safety tests: import scan for boto3 usage outside the provider package; wrapper reflection vs `ALLOWED_OPERATIONS`; IAM-policy equality; and `tests/test_no_mutating_calls.py` audit of SDK-like and AWS CLI mutating verbs outside static templates.
